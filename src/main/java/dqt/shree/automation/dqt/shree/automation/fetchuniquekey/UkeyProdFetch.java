...
public class UkeyProdFetch {

    static final Logger logger= Logger.getLogger(UkeyProdFetch.class.getName());
    ...
    public static void uniquekey() throws JSONException,IOException {
        ...
        StringEntity[] postRequests = getRequestArray("C:\\Dataid\\dataidPROD.txt");
        List<String> words = readArray("C:\\Dataid\\dataidPROD.txt");
        ...
        for (int i = 0; i < postRequests.length; i++) {
            ...
            HttpPost postRequest = new HttpPost("http://dataselfservice.bcdtravel.com:8080//DQTAPIProject-0.0.1-SNAPSHOT/InvApi/getFilteredTransErrors/1/500");
        ...
        try(FileWriter writer = new FileWriter("C:\\Uniquekey\\Prod\\uniquekeyprod" + " " + dateFormat.format(date) + ".txt", true)) {
    ...
    private static void produky(HashSet<String> keystore, JSONArray objJSONArray) {
        String v0;
        try {
            for (int l = 0; l <= 5; l++) {
